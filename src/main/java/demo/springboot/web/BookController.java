...
public class BookController {
     ...
     * 处理 "/book/{id}" 的 GET 请求，用来删除 Book 信息
     */
    @RequestMapping(value = "/download0/{id}", method = RequestMethod.GET)
    public void downloadBook0(@PathVariable Long id,  HttpServletResponse response) {
            bookService.downloadBook(id,response);
    }
    /**
     * 下载Book
     * 处理 "/book/{id}" 的 GET 请求，用来删除 Book 信息
    ...
    public void downloadBook(@PathVariable Long id,  HttpServletResponse response) {
        try {
            bookService.downloadBookZip(id,response,1);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void downloadBook2(@PathVariable Long id, HttpServletResponse response)  {
        try {
            bookService.downloadBookZip(id,response,0);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
