...
public class Excel2TemplateUtils {
     ...
     */
    public static void process(Object data, InputStream templatePath, OutputStream os) {
        if (data == null ) {
            return;
        }
        try {
//            OPCPackage pkg = OPCPackage.open(templatePath);
            HSSFWorkbook wb = new HSSFWorkbook(templatePath);
...
//            pkg.close();
     ...
     * @param templatePath 模板文件路径
     */
    public static HSSFWorkbook process2(Object data, InputStream templatePath) {
        if (data == null ) {
            return null;
        }
        try {
    ...
    private static void setCellValue(Cell cell, Object value) {
        if (value == null) {
            ...
            String endStr = "";
            endStr = cell.getStringCellValue().substring(cell.getStringCellValue().lastIndexOf("}"));
            if (StringUtils.isNotBlank(endStr)){
                endStr = endStr.substring(1);
            }
            cell.setCellValue(value.toString()+endStr);
        }
    }
